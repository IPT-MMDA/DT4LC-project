"""Phase 2 Tests - LLM-Powered Planner."""
...
def test_format_registry_for_llm(mock_registry: Registry) -> None:
...
def test_estimate_confidence_high(simple_context: ContextUnderstanding) -> None:
    """Test confidence estimation for simple request."""
    confidence = estimate_plan_confidence(simple_context)

    # Should be high confidence (keywords + inputs + outputs)
    assert confidence >= 0.7


def test_estimate_confidence_low(complex_context: ContextUnderstanding) -> None:
    """Test confidence estimation for complex request."""
    confidence = estimate_plan_confidence(complex_context)

    # Should be low confidence (no keywords, no clear inputs/outputs)
    assert confidence < 0.7


def test_template_planner(simple_context: ContextUnderstanding, mock_registry: Registry) -> None:
...
def test_llm_planner_with_mock(simple_context: ContextUnderstanding, mock_registry: Registry) -> None:
    ...
    # Mock response with valid plan JSON
...
def test_llm_planner_invalid_json(simple_context: ContextUnderstanding, mock_registry: Registry) -> None:
...
def test_llm_planner_missing_component(simple_context: ContextUnderstanding, mock_registry: Registry) -> None:
    ...
    # Mock response with invalid component ID
    mock_response = LLMResponse(
        text="""{
  "steps": [
...
def test_llm_planner_markdown_json(simple_context: ContextUnderstanding, mock_registry: Registry) -> None:
...
def test_hybrid_planner_uses_template(simple_context: ContextUnderstanding, mock_registry: Registry) -> None:
    """Test hybrid planner uses template for high confidence."""
    # Simple context has high confidence -> should use template
...
def test_hybrid_planner_uses_llm(complex_context: ContextUnderstanding, mock_registry: Registry) -> None:
    """Test hybrid planner attempts LLM for low confidence."""
    # Complex context has low confidence -> should try LLM
    # Will fall back to template if LLM unavailable

    plan_result = plan(complex_context, mock_registry, use_llm=True)

    assert plan_result is not None
    assert len(plan_result.steps) >= 0  # May be empty for complex context with template fallback


def test_hybrid_planner_force_template(complex_context: ContextUnderstanding, mock_registry: Registry) -> None:
...
def test_llm_planner_empty_steps(simple_context: ContextUnderstanding, mock_registry: Registry) -> None:
    ...
    # Mock response with empty steps
    mock_response = LLMResponse(
...
def test_confidence_scoring_edge_cases() -> None:
    ...
    assert confidence == 1.0  # All bonuses: 0.3 (keywords) + 0.3 (known pattern) + 0.2 (inputs) + 0.2 (outputs)
